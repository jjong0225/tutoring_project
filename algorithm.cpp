...
void init_graph(MetroTreeNode * root_ptr){
    ...
    } //tree를 순회하며 station node 초기화  및 생성
     ...
     while(!q.empty()){
		...
		for(int i=0;i<list_size;++i){
            ...
            int nex_id=cur.get_id();
             int nex_weight=cur.get_time_weight();  //10으로 먼저 초기화함
            int nex_line=cur.get_line();       
            it_node=Node_map.find(nex_id);
            Node newtemp;

            if(it_node ==Node_map.end()){ //출발이 없는 역 (트리에 생성되지 않은 역)             
...
pair<string,vector<pair<int,int>>> track_path(int departure_id,int arrive_id, map<int ,tuple<int,int,int>> path){ 
    ...
    while(1){
        node_id=get<0>(path[node_id]); // 정류장 번호     
        int newrail=get<1>(path[node_id]); // 노선 정보       
        if(past_rail!=newrail &&node_id!=departure_id){ // 이전 노선 정보와 새로 받은 노선 정보가 다를 때
   ...
   for(int i=0;i<size;i++){
       ...
       if( cur.second.departure_data[j] >=start_time && min_departure>cur.second.departure_data[j])
        { int param=cur.second.departure_data[j];
           ...
           min_departure=cur.second.departure_data[j];} // 도착 지점이 같더라도 더 빠른 정보를 기록              
             } 
        }
   }
...
pair<map<int,int>, map <int,tuple<int,int,int>>> trans_dijkstra(int departure_id ,int arrive_id,int time){
   ...
   map <int, int> dist;
   map<int,tuple<int,int,int>> path; //정류장,노선, 소요시간  
   ...
   for(int i=1;i<=station_num;i++){  // travel 하면서 모든 노드를 초기화 해야함   
       ...
       dist.insert(make_pair(id,INF));  // KEY: ID Value: 소요시간
       path.insert(make_pair(id,make_tuple(-1,-1,-1))); //KEY: ID , VALUE (연결 역 id, line, time)
       cnt++;
       it_new++;
   }

   dist[departure_id]=0; 
   q.push(make_tuple(0,departure_id,0)); // 시작점을 처음으로 우선순위 큐에 추가
...
pair<int, pair<string,vector<pair<int,int>>>>find_max_departure_time(int A_station_id , int B_station_id ,int now_time,int deadline_time){
   ...
   int departure=now_time;
    ...
    int tot=0;
    int before=0;
    pair<int,int> gap=make_pair(0,0); // 출발 시간, 대기 시간
...
tuple<string,Track_info,Track_info> find_optimized_schedule_path(int A_station, int B_station,int end_schedule_time, int next_schedule_time){
    ...
    int estimated_departure=end_schedule_time; // 출발 시간 가정
    int estimated_arrive=next_schedule_time; // 도착 예정 시간 가정 (다음 스케줄 시작 시간으로 추후 변경)
